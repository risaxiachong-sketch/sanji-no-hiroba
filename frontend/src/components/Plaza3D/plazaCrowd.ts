...
/**
 * Splits `totalVisitors` across the topics as evenly as possible, so the crowd
 * size can track the plaza's actual headcount instead of a fixed 48.
 */
function distributeAcrossTopics(totalVisitors: number, topicCount: number): number[] {
  const base = Math.floor(totalVisitors / topicCount)
  const remainder = totalVisitors - base * topicCount
  return Array.from({ length: topicCount }, (_, index) => base + (index < remainder ? 1 : 0))
}
...
export class PlazaCrowd {
  ...
  constructor(totalVisitors: number, seed = 20260721) {
    ...
    const topicCounts = distributeAcrossTopics(Math.max(0, totalVisitors), TOPICS.length)

    TOPICS.forEach((topic, topicIndex) => {
      const memberCount = topicCounts[topicIndex]
      ...
      for (let member = 0; member < memberCount; member += 1) {
        ...
        const slotAngle = (member / memberCount) * Math.PI * 2 + this.random() * 0.35 - 0.175
        ...
        this.visitors.push({
          topic: topic.id,
          variant: this.visitors.length % VISITOR_VARIANTS,
          x: startX,
          z: startZ,
          velocityX: 0,
          velocityZ: 0,
