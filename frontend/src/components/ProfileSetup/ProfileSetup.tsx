...
export function ProfileSetup({ onComplete }: Props) {
  ...
  const handleSubmit = () => {
    if (!isValid) return
    onComplete({ userId: '', nickname: nickname.trim(), childAgeGroup, avatarId: '' })
  }
  ...
  return (
    <div className={styles.page}>
      ...
      <form className={styles.card} onSubmit={(event) => { event.preventDefault(); handleSubmit() }}>
        <div className={styles.field}>
          <div className={styles.labelRow}>
            <label className={styles.label} htmlFor="nickname">ニックネーム</label>
          </div>
          <div className={styles.inputShell}>
            <input id="nickname" type="text" className={styles.input} maxLength={20} value={nickname} onChange={(event) => setNickname(event.target.value)} placeholder="例: もこママ" />
          </div>
        ...
        <div className={styles.field}>
          <div className={styles.labelRow}>
            <label className={styles.label} htmlFor="childAgeGroup">お子さんの年齢</label>
          </div>
          <div
          ...
          >
            ...
            {isAgeMenuOpen && (
              <div className={styles.optionPanel} role="listbox" aria-labelledby="childAgeGroup">
                <button type="button" className={`${styles.optionButton} ${childAgeGroup === '' ? styles.optionSelected : ''}`} role="option" aria-selected={childAgeGroup === ''} onClick={() => { setChildAgeGroup(''); setIsAgeMenuOpen(false) }}>選択してください</button>
                {AGE_GROUPS.map((group) => (
                  <button key={group.value} type="button" className={`${styles.optionButton} ${childAgeGroup === group.value ? styles.optionSelected : ''}`} role="option" aria-selected={childAgeGroup === group.value} onClick={() => { setChildAgeGroup(group.value); setIsAgeMenuOpen(false) }}>
                    {group.label}
                  </button>
                ))}
              </div>
